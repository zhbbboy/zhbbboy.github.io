...
- 它的左右子树也分别为二叉搜索树
![](../../_Pasted_Image/Pasted%20image%2020260830134956.png)
...
![](../../_Pasted_Image/Pasted%20image%2020260830135012.png)
**递归：**
##### (1) 左右子树本身是二叉搜索树
...
##### (2) 以根节点为核心的分层定义
+ 当前树的性质由当前根节点与左右子树的性质共同决定；
+ 左子树和右子树本身可以被视为规模更小的二叉搜索树，这种嵌套结构直接表明递归定义的存在。
...
- 原因 1: 明确默认构造需求
	如果没有声明任何构造函数，编译器会自动生成默认构造函数，且会将 _ root 初始化为 nullptr（因为你用了类内初始化 Node* _ root = nullptr;）。但如果你显式定义了其他构造函数（比如拷贝构造、赋值重载），编译器将不再自动生成默认构造函数。为避免丢失默认构造函数的功能，就需要显式使用 = default;。

- 原因 2: 保持代码的可读性与语义  
    通过显式声明 `BSTree() = default;`，代码更直观地表明：  
    _“这个类的默认构造行为是由编译器生成的，我没有进行额外的修改。”_
...
现在我们要将 `tree1` 赋值为 `tree2`，即 `tree1`
...
1. 创建临时变量 `temp`：`temp` 保存 `this->_root`（即 `tree1` 的 `_root`），此时 `temp = 5`。
2. 将 **`t._root`** 赋给 `this->_root`：此时 `this->_root` 指向 `t._root` 的值（即 `10`），也就是 `tree2` 的根节点。
3. 将 **`temp`** 赋给 `t._root`：最后，`t._root` 变为 `5`，即之前 `tree1` 的根节点。
...
关键点总结：
1. **临时副本**：通过传值的方式，将 `tree2` 的副本传递给 `t`，`t` 独立于 `tree2`。
2. **高效的 `swap`**：`swap` 交换了 `tree1` 和 `t` 的根节点指针，而不是深拷贝整个树，效率更高。
3. **资源管理**：由于 `t` 是传入的临时变量，赋值过程结束后 `t` 会被自动销毁，释放旧的 `tree1` 的内存，避免了手动 `Destroy` 的麻烦。
4. **链式赋值**：`return *this` 支持链式调用，如 `tree1 = tree2 = tree3;`。
...
### InOrder()为何这样设计？
`InOrder()` 函数设计的目的是为了**封装实现细节**，让用户只需调用接口函数即可完成对二叉树的中序遍历，而不必了解树的内部数据结构。这种设计的优势在于以下几点：
...
这种封装遵循了**面向对象编程中的封装原则**，屏蔽了用户不需要知道的复杂实现。
...
用户调用 `InOrder()` 的方式保持不变，代码改动对外透明。这种设计符合**模块化设计**的思想。
...
- 最多查找**高度次**，走到到空，还没找到，这个值不存在
![](../../_Pasted_Image/Pasted%20image%2020260830141128.png)
另外需要注意的是  
①最大元素一定是在树的最右分枝的端结点上。  
②最小元素一定是在树的最左分枝的端结点上。![](../../_Pasted_Image/Pasted%20image%2020260830141135.png)
```cpp
//find函数。 用k，关键字的意思 
...
### 2.插入
![](../../_Pasted_Image/Pasted%20image%2020260830141158.png)
...
![](../../_Pasted_Image/Pasted%20image%2020260830141337.png)

- **d情况（要删除的结点有左、右孩子结点） ——替换法删除**

先假删除根节点8.![](../../_Pasted_Image/Pasted%20image%2020260830141348.png)
用7替换待删除节点![](../../_Pasted_Image/Pasted%20image%2020260830141358.png)

用10替换待删除节点![](../../_Pasted_Image/Pasted%20image%2020260830141405.png)
...
下面一张图也可以帮助理解。
![](../../_Pasted_Image/Pasted%20image%2020260830141458.png)
...
![](../../_Pasted_Image/Pasted%20image%2020260830141520.png)
![](../../_Pasted_Image/Pasted%20image%2020260830141544.png)
## 六、应用——KV模型
...
string strs[] = { "苹果", "西瓜", "苹果", "樱桃", "苹果", "樱桃", "苹果", "樱桃", "苹果" };
	...
	countTree.InOrder();
```
![](../../_Pasted_Image/Pasted%20image%2020260830141647.png)
...
1. 第一处 `template <class K>`
```cpp
template <class K>
struct BSTreeNode
```
这一部分定义了一个模板结构体 `BSTreeNode`，它接受一个模板参数 `K`，用来指定二叉搜索树节点的关键字 类型。模板参数 `K` 是用来描述节点内部的 `_key` 数据成员类型。

2. 第二处 `template <class K>`
```cpp
template <class K>
class BSTree
```
...
- 在 `BSTree` 中使用 `BSTreeNode<K>` 时，需要显式传递模板参数 `K`，以表明 `BSTreeNode` 的模板实例化。
...
****`bool`** 是一种数据类型**
- **描述**: `bool` 是一种内置的布尔类型，用来表示逻辑上的真假值，取值范围是 `true` 或 `false`。
...
****`status`** 是一种抽象的状态表示**
- **描述**: `status` 一般用于表示系统、进程、任务、操作等的状态，通常不局限于 `true` 和 `false` 两种值。
...
**3. 典型区别**
![](../../_Pasted_Image/Pasted%20image%2020260830142028.png)

**4. 选择依据**
- **简单的真假判断**:
    - 如果某个结果仅需表示 **“是或否”**（例如操作是否成功），选择 `bool`。
- **描述复杂状态**:
    - 如果需要提供更详细的上下文信息，或者状态有多个可能值，选择 `status` 或类似的机制。
